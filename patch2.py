...
# Replace toggle_language area
target_toggle = """                        urdu_shapes = [
                            URDU_TEXT['shape_rect'],
                            URDU_TEXT['shape_tri'],
                            URDU_TEXT['shape_quad_tri'],
                            URDU_TEXT['shape_quad_avg']
                        ]"""
replace_toggle = """                        urdu_shapes = [
                            URDU_TEXT['shape_rect'],
                            URDU_TEXT['shape_tri'],
                            URDU_TEXT['shape_quad_tri'],
                            URDU_TEXT['shape_quad_avg']
...
# Extract everything between def build_area_tab and def add_conversion
start_idx = content.find('    def build_area_tab(self, parent):')
end_idx = content.find('    def add_conversion(self):')

methods_implementation = """    def build_area_tab(self, parent):
...
content = content[:start_idx] + methods_implementation + "\n" + content[end_idx:]

with open(file_path, 'w', encoding='utf-8') as f:
    f.write(content)
print("Patch applied for 10-sided polygons")
